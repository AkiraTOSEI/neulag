...
'''
This code is based on the repository below.
https://github.com/BensonRen/AEM_DIM_Bench/blob/main/Data/Chen/generate_chen.py
'''
...
def X_Curve_Y_thicks(config, mX, mY, nPt, fname, pick_thick=-1):
    pathX = f"./{fname}_x.csv"
    pathY = f"./{fname}_y.csv"
...
def generate(ndata, random_seed1=2018, random_seed2=42, fname='data'):
    config = GraSi3N4_init(random_seed1,'v1')
    ...
    pathZ = GraSi3N4_sample(ndata, random_seed2, sKeyTitle, config, n_dict)
    ...
    x,y = X_Curve_Y_thicks(config, mX, mY, nPt, fname)
    os.remove(pathZ)
    return x,y
...
if __name__ == '__main__':
    ...
    generate(ndata, random_seed1, random_seed2,fname= 'data')
    print('The generation process of generating {} takes {}s'.format(ndata, time.time() - start))
    
    ## for medium surrogator model
    #generate(200000, random_seed1, random_seed2,fname= 'data-mid')
    #print('The generation process of generating {} takes {}s'.format(ndata, time.time() - start))
    ...
    random_seed1 = 2022
    random_seed2 = 48
    ndata = 500    # Test set (half would be taken)

    generate(ndata, random_seed1, random_seed2,fname= 'test')
    print('The generation process of generating {} takes {}s'.format(ndata, time.time() - start))
