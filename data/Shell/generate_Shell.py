...
def combine_multi_processing(num_files, fname):
    """
    ...
    print(fname)
    np.savetxt(f'{fname}_x.csv', data_x_full, delimiter=',')
    np.savetxt(f'{fname}_y.csv', data_y_full, delimiter=',')



def create_data(ndata:int,fname:str='data',num_cpu=4):
    # # Normal bound
    start = time.time()
    num_cpu = 4
    try: 
    ...
    combine_multi_processing(num_cpu,fname)

if __name__ == "__main__":
    create_data(50000,fname='data')
    create_data(500,fname='test')
    
    # for medium surrogator model
    #create_data(200000,fname='data-mid')
