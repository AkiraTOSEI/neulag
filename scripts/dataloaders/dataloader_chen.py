...
from utils.task_utils import X_normalization_Chen
...
class Dataset4FowardModel(torch.utils.data.Dataset):
    def __init__(self, X, Y):
        ...
        self.X = X_normalization_Chen(self.X)
...
def Dataloader4FowardModel(
    cfg: omegaconf.dictconfig.DictConfig, random_state: int = 2022
    ...
    train_loader = DataLoader(
        Dataset4FowardModel(X_train, Y_train),
        batch_size=cfg.train.batch_size,
        shuffle=True,
    )
    valid_loader = DataLoader(
        Dataset4FowardModel(X_valid, Y_valid),
        batch_size=cfg.train.batch_size,
        shuffle=False,
    )
...
class Dataset4InverseModel(torch.utils.data.Dataset):
    def __init__(self, X: np.array, Y: np.array, cfg: omegaconf.dictconfig.DictConfig):
        super().__init__()
        self.X, self.Y = X.astype(np.float32), Y.astype(np.float32)
        self.X = X_normalization_Chen(self.X)
        num = cfg.inverse_problem.eval_batch_size
        ...
        if mode == "batch_target":
            ...
            self.X = self.X[target_id : target_id + 1]
            self.Y = self.Y[target_id : target_id + 1]
        elif mode == "single_target_for_multi_solution":
            assert type(target_id) == int
            self.X = self.X[target_id : target_id + 1]
            self.X = np.concatenate([self.X] * num, axis=0)
            self.Y = self.Y[target_id : target_id + 1]
            self.Y = np.concatenate([self.Y] * num, axis=0)
        else:
            raise Exception(
