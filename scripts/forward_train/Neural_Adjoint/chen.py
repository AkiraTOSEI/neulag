...
from dataloaders.dataloader_chen import Dataloader4FowardModel
...
def train_forward_model(cfg: omegaconf.dictconfig.DictConfig, test: bool = False):
    """train forward model
    Args:
    ...
    trainer.fit(
        pl_model, train_dataloader=train_loader, val_dataloaders=valid_loader,
    )
    torch.save(
